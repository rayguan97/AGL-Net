...
def train(cfg: DictConfig, job_id: Optional[int] = None):
    ...
    config_path = osp.join(EXPERIMENTS_PATH, cfg.experiment.name, "config.yaml")
    if cfg.resume and osp.exists(config_path):
            with open(config_path, "r") as fp:
                cfg = OmegaConf.create(fp.read())
    if rank == 0:
    ...
    data = data_modules[cfg.data.get("name", "carla")](cfg.data)
...
@hydra.main(
    config_path=osp.join(osp.dirname(__file__), "conf"), config_name="orienternet_kitti_lidar"
)
