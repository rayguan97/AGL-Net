...
class DataAgent(AutoPilot):
    ...
    def reset_weather_param(self, weather):
        if weather:
            self.weathers = {
                'Clear': carla.WeatherParameters.ClearNoon,
                'Cloudy': carla.WeatherParameters.CloudySunset,
    ...
    def sensors(self):
        ...
        if self.save_path is not None:
            result += [
                    ...
                    #     'x': 0, 'y': 0.0, 'z':150,
                    #     'roll': 0.0, 'pitch': -90.0, 'yaw': 0.0,
                    #     'width': 5000, 'height': 5000, 'fov': 110,
                    #     'id': 'rgb_topdown'
                    # },
                    {
    ...
    def tick(self, input_data):
        ...
        if self.save_path is not None:
            ...
            topdown_rgb = cv2.cvtColor(input_data['rgb_topdown'][1][:, :, :3], cv2.COLOR_BGR2RGB)
    ...
    def save_sensors(self, tick_data):
        ...
        img = cv2.cvtColor(tick_data['topdown_rgb'],cv2.COLOR_RGB2BGR)
        cv2.imwrite(str(self.save_path / 'topdown_rgb' / ('%04d.png' % frame)), img)
